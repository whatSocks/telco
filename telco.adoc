...
RETURN a.resource_id AS `RESOURCE`, r.status AS `LINK`, b.resource_id AS `RESOURCE`
----
//table
...
_question: is there a way to break ties with shortestpath?_
_question: is there a way find shortestPath while specifying properties of relationships?_
