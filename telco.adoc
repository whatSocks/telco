...
^ replace this image with an image of this network, update names. 

Let's find bridges in our network

[source,cypher]
----
MATCH path = (a:crew)-[bridge*1]-(b:crew)
WHERE NOT (a)-[*2..3]-(b)
...
_note - edit query and prose below so that we know which end the service is coming from_
